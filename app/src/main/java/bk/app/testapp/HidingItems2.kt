...
interface HidingItem {
    var isHide: Boolean
}

interface HidingItems2 {
...
class HidingItemsAdapter2(private val callback: ListUpdateCallback?) : HidingItems2,
    ListUpdateCallback {
...
open class HidingList2<T> private constructor(
    private val hidingItems2: HidingItemsAdapter2
) : AbstractList<T>(), HidingItems2 by hidingItems2 {
    ...
    constructor(
        ...
        HidingItemsAdapter2(listUpdateCallback)
    ) {
        if (diffCallback != null) {
            differ =
                AsyncListDiffer<T>(hidingItems2, AsyncDifferConfig.Builder(diffCallback).build())
        }
        submitList(list)
    }

    // list
    override val size get() = hidingItems2.getShownPos(list.size)

    override fun get(index: Int): T = list[hidingItems2.getRawPos(index)]
